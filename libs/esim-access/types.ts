...
export interface BaseResponse<T = any> {
  success: string; // "true" or "false" strings in doc
...
export interface OrderRequest {
  ...
  packageInfoList: {
    packageCode?: string;
    slug?: string; // Doc says "packageCode or slug"
    count: number;
    price?: number;
    periodNum?: number; // Not in example but mentioned in prompt rules
  }[];
}
...
export interface WebhookBase {
  ...
  notifyId?: string; // optional in some examples
  eventGenerateTime?: string; // optional in some examples
}
...
export interface WebhookOrderStatus extends WebhookBase {
  ...
  content: {
    orderNo: string;
    orderStatus: string; // "GOT_RESOURCE" etc
  };
}
