...
export function PlanComparison({ plans, onClose, onRemove }: PlanComparisonProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <Card className="w-full max-w-6xl max-h-[90vh] overflow-hidden bg-[var(--voyo-card)] border-[var(--voyo-border)]">
        <CardHeader className="border-b border-[var(--voyo-border)]">
        ...
        <CardContent className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
            ...
            {displayPlans.map((plan) => {
              ...
              return (
                <div
                  ...
                  className="bg-[var(--voyo-bg-light)] border border-[var(--voyo-border)] rounded-lg p-4 flex flex-col h-full"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-[var(--voyo-muted)] line-clamp-2">
                        {plan.name}
                      </p>
                    </div>
                    {onRemove && (
                      <button
                        ...
                        className="text-[var(--voyo-muted)] hover:text-white ml-2"
                      >
                        <X className="h-4 w-4" />
                      </button>
                    )}
                  ...
                  <div className="space-y-3 flex-1">
                    <div>
                      <div className="text-xs text-[var(--voyo-muted)] mb-1">Price</div>
                    ...
                    <div>
                      <div className="text-xs text-[var(--voyo-muted)] mb-1">Duration</div>
                    ...
                    <div>
                      <div className="text-xs text-[var(--voyo-muted)] mb-1">Data</div>
                      <div className="text-sm text-white">
                        {gb.toFixed(1)} GB
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="text-xs text-[var(--voyo-muted)] mb-1">Value</div>
                      <div className="text-sm text-[var(--voyo-accent)] font-semibold">
                        {value.toFixed(2)} GB/$
                      </div>
                    </div>
                    ...
                    {plan.speed && (
                      <div>
                        <div className="text-xs text-[var(--voyo-muted)] mb-1">Speed</div>
                  ...
                  <Link href={`/plans/${plan.packageCode}`} className="mt-4">
                    <Button className="w-full bg-[var(--voyo-accent)] hover:bg-[var(--voyo-accent)]/80 text-white">
                      View Details
                    </Button>
                  </Link>
                </div>
          ...
          {plans.length > maxPlans && (
            <div className="mt-4 text-center text-sm text-[var(--voyo-muted)]">
