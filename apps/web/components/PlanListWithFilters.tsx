...
export function PlanListWithFilters<T extends PlanBase>({ 
  ...
  return (
    <div className="space-y-6">
      {/* Filter Bar */}
      <div className="bg-[var(--voyo-card)] border border-[var(--voyo-border)] rounded-xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-lg">
        <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 no-scrollbar">
          
          {/* Sort Control */}
          <div className="flex items-center gap-2 min-w-fit">
            <ArrowDownUp className="h-4 w-4 text-[var(--voyo-muted)]" />
            <span className="text-sm text-[var(--voyo-muted)] hidden sm:inline">Sort by:</span>
            <div className="relative">
              <select 
                value={sort} 
                onChange={(e) => setSort(e.target.value as SortOption)}
                className="appearance-none bg-[var(--voyo-bg)] text-white text-sm rounded-lg border border-[var(--voyo-border)] pl-3 pr-8 py-2 focus:outline-none focus:border-[var(--voyo-accent)]"
              >
              ...
              <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--voyo-muted)] pointer-events-none" />
            </div>
          </div>

          <div className="h-6 w-px bg-[var(--voyo-border)] hidden md:block" />

          {/* Quick Filters (Desktop) */}
          <div className="hidden md:flex items-center gap-3">
             <Filter className="h-4 w-4 text-[var(--voyo-muted)]" />
          ...
          >
            ...
            {activeFilterCount > 0 && (
              <span className="bg-[var(--voyo-accent)] text-white text-[10px] px-1.5 py-0.5 rounded-full">
                {activeFilterCount}
              </span>
            )}
          </Button>
        ...
        {(sort !== "price_asc" || durationFilter !== "all" || dataFilter !== "all") && (
          <Button 
            ...
            className="text-[var(--voyo-muted)] hover:text-white flex items-center gap-2 whitespace-nowrap"
          >
      ...
      {isFiltersOpen && (
         <div className="md:hidden bg-[var(--voyo-card)] border border-[var(--voyo-border)] rounded-xl p-4 space-y-4 animate-in slide-in-from-top-2">
            <div className="space-y-2">
               <label className="text-xs text-[var(--voyo-muted)] uppercase font-bold">Duration</label>
               <select 
                  value={durationFilter} 
                  onChange={(e) => setDurationFilter(e.target.value as DurationFilter)}
                  className="w-full bg-[var(--voyo-bg)] text-white text-sm rounded-lg border border-[var(--voyo-border)] px-3 py-2"
               >
                  <option value="all">All Durations</option>
                  <option value="1-7">1 - 7 Days</option>
                  <option value="8-15">8 - 15 Days</option>
            ...
            <div className="space-y-2">
               <label className="text-xs text-[var(--voyo-muted)] uppercase font-bold">Data</label>
               <select 
                  value={dataFilter} 
                  onChange={(e) => setDataFilter(e.target.value as DataFilter)}
                  className="w-full bg-[var(--voyo-bg)] text-white text-sm rounded-lg border border-[var(--voyo-border)] px-3 py-2"
      ...
      {filteredPlans.length === 0 ? (
         <div className="text-center py-20 bg-[var(--voyo-card)] rounded-xl border border-[var(--voyo-border)]">
            <p className="text-[var(--voyo-muted)]">{emptyMessage}</p>
            <Button variant="link" onClick={resetFilters} className="mt-2 text-[var(--voyo-accent)]">
              Clear filters
            </Button>
         </div>
      ) : (
