...
import { CheckCircle2, QrCode, Mail, ExternalLink, ArrowRight, Smartphone, Download, Clock } from "lucide-react";
    ...
    const fetchOrderAndFireConversion = async () => {
      try {
        ...
        if (order && order.id && order.amountCents) {
          ...
          try {
            const orderDetails = await safeFetch<OrderDetails>(`${apiUrl}/orders/${order.id}`, { showToast: false });
          ...
          <Card className="bg-[var(--voyage-card)] border-[var(--voyage-border)]">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button
                  variant="outline"
                  className="flex-1 border-[var(--voyage-border)] bg-[var(--voyage-bg-light)] text-white hover:bg-[var(--voyage-card)]"
                  onClick={() => window.open(esimProfile.qrCodeUrl, '_blank')}
                >
              ...
              <Link href={`/my-esims/${esimProfile.iccid}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          {orderData && (
            <Link href={`/my-esims?order=${orderData.id}`} className="flex-1">
              <Button 
                variant="outline" 
                className="w-full border-[var(--voyage-border)] bg-[var(--voyage-bg-light)] text-white hover:bg-[var(--voyage-card)] py-6 text-lg"
              >
                Track Order
              </Button>
            </Link>
